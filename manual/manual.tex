...
\subsection{O que \`e um SYN Flood?}
\begin{enumerate}[itemsep=2pt]
  \item Atacante envia muitos pacotes TCP SYN (pedidos de conex\~ao) para um servidor
  \item Servidor responde com SYN-ACK, aguardando o ACK final do cliente (three-way handshake)
  \item Atacante nunca envia o ACK final (ou falsifica IP de origem)
  \item Servidor fica com muitas conex\~oes meio-abertas (\textit{half-open})
  \item Buffer de conex\~oes pendentes enche, servidor nega novas conex\~oes leg\'itimas
\end{enumerate}
...
\subsection{Output (exemplo)}
\begin{lstlisting}[style=bash]
Log Analyzer - Analise de Registos
...
\section{Messenger}
Servi\c{c}o seguro de troca de mensagens entre cliente e servidor usando criptografia sim\'etrica e assim\'etrica.
...
\subsection{Servidor}
\begin{lstlisting}[style=bash]
# Iniciar servidor na porta padrao (9009)
...
\subsection{Cliente - Enviar Mensagem}
\begin{lstlisting}[style=bash]
...
\subsection{Cliente - Listar Mensagens}
\begin{lstlisting}[style=bash]
python3 messenger.py list --user alice --host 127.0.0.1 --port 9009
\end{lstlisting}

\subsection{Cliente - Descarregar Mensagem}
\begin{lstlisting}[style=bash]
...
\subsection{Cliente - Apagar Mensagem}
\begin{lstlisting}[style=bash]
...
\subsection{Cliente - Backup Sim\'etrico}
\begin{lstlisting}[style=bash]
...
\subsection{Cliente - Backup Assim\'etrico}
\begin{lstlisting}[style=bash]
python3 messenger.py backup-asym --host 127.0.0.1 --port 9009
\end{lstlisting}

\subsection{Caracter\'isticas Principais}
\begin{itemize}
  \item Mensagens encriptadas com criptografia h\'ibrida (Fernet + RSA)
  \item Suporte a m\'ultiplos utilizadores
  \item Arquivo de mensagens no servidor
  \item Backup com encripta\c{c}\~ao sim\'etrica (com senha) ou assim\'etrica (com chave p\'ublica)
  \item Descarga seletiva de mensagens
  \item Elimina\c{c}\~ao de mensagens apenas por participantes
\end{itemize}
...
\section{Ficheiros de Log e Relat\'orios}
\begin{itemize}
  \item Logs: \texttt{src/logs/}
  \item Relat\'orios: \texttt{reports/}
  \item Dados do Password Manager: \texttt{src/scripts/}
  \item Dados do Messenger: \texttt{src/data/messages/}, \texttt{src/data/keys/}, \texttt{src/data/backups/}
\end{itemize}
